...
```{r}
title <- speech_page %>%
    html_element("#main-content > header > div.gov_sublayout__title > h1") %>%
    html_text()
...
speech_page %>%
    html_element("#sg-meta__publication-date") %>%
    html_text() 
...
location <- speech_page %>%
    html_element("div.gov_sublayout__metadata > dl > div:nth-child(5) > dd > strong") %>%
    html_text()
...
abstract <- speech_page %>%
    html_element("#main-content > header > div.gov_sublayout__content > p") %>%
    html_text()
...
```{r output.lines=11}
text <- speech_page %>% 
    html_elements("#preamble > div > p:nth-child(n+2)") %>%
    html_text() %>%
    list()
